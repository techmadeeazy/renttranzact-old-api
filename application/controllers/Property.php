...
class Property extends REST_Controller
{
    ...
    public function search_post()
    {
        ...
        $propertyData = $this->Base_model->get_many('property_listings', "(display = 'public' AND status = 'approved' AND active = 1) AND (description = '%$keyword%' OR title = '%$keyword%' OR state = '$state' OR lga ='$lga')");
        $responseData = [];
        $this->load->model('UserAuth_model');
        $this->load->model('UserProfile_model');
        foreach ($propertyData as $pd) {
